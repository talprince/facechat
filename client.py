...
class Client:
    ...
    def continue1(self, my_socket):
        ...
        # recieving from the server index of client
        #  from server to set the ports
        try:
        ...
        self.my_socket.send(socket.gethostbyname
    ...
    def run(self):
        """
        Sends and recieves the landmarks (sends what returned from
        find_landmarks and recieves from socket)
        This function loops forever.
        """
        print("1")
        # Getting the landmarks and making them byte string,
        # if didn't find face encoding the message noface
        ...
        # if decoding works that means its a string, which is
        # 'noface' if it doesnt its the numpy array of coords
        try:
            data = data.decode()
        except Exception:
            self.landmarks = np.fromstring\
                (data, dtype=int).reshape(68, 2)
    ...
    def find_landmarks(self):
        """
        Finds and returns the face landmark points.
        If can't detect a face returns 'noface'.
        """
    ...
    def getgrayimg(self):
        ...
        """
        # ret will get true or false and img
        # will get the image from video captured
    ...
    def shape_to_np(self, shape, dtype="int"):
        """
        Takes Shape object from dlib face landmark detector
        and returns numpy array of the coordinates
        """
        # initialize the list of (x, y)-coordinates
        coords = np.zeros((68, 2), dtype=dtype)
        # loop over the 68 facial landmarks and
        # convert them to a 2-tuple of (x, y)-coordinates
        for i in range(0, 68):
    ...
    def draw_shape(self, endline, startn, endn):
        """
        Draws the face part by given args: which
        landmarks and if to close the part
        by line between first and last landmark
        """
        startn -= 1
        for x in range(startn, endn):
            if x > startn:
                self.canvas.create_line(self.landmarks[x, 0],
                                        self.landmarks[x, 1], xr, yr, width=2)
